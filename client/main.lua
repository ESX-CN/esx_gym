...
local blips = {
    {title = _U('title'),colour = 7,id = 311,x = -1201.2257,y = -1568.8670,z = 4.6101}
}
...
local rentbike = {{x = -1199.1164, y = -1584.5972, z = 4.3249}}
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
                hintToDisplay(_U('hire_bike'))
                if IsControlJustPressed(0, Keys['E']) then -- "E"
                    if IsPedInAnyVehicle(PlayerPedId()) then
                        ESX.ShowNotification(_U('have_vehicle'))
                    else
                        OpenBikeMenu()
                    end
                end
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
                hintToDisplay(_U('exercise_arms'))
                if IsControlJustPressed(0, Keys['E']) then
                    if training == false then
                        TriggerServerEvent('esx_gym:checkChip')
                        ESX.ShowNotification(_U('preparing'))
                        Citizen.Wait(1000)
                        if membership == true then
                            ...
                            ESX.ShowNotification(_U('need_to_rest'))
                            -- TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
                            training = true
                        elseif membership == false then
                            ESX.ShowNotification(_U('need_membership'))
                        end
                    elseif training == true then
                        ESX.ShowNotification(_U('need_rest'))
                        resting = true
                        CheckTraining()
                    end
                end
...
function CheckTraining()
    if resting == true then
        ESX.ShowNotification(_U('are_resting'))
        resting = false
        Citizen.Wait(60000)
        training = false
	end
	...
	if resting == false then
		ESX.ShowNotification(_U('can_exercise'))
	end
end
...
function OpenGymMenu()
    ...
    ESX.UI.Menu.Open('default',GetCurrentResourceName(),'gym_menu',{
		title = _U('title'),
		elements = {
			{label = _U('menu_shop'), value = 'shop'},
			{label = _U('business_hours'), value = 'hours'},
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open('default',GetCurrentResourceName(),'gym_shop_menu',{
		title = _U('title') .. ' - ' .. _U('menu_shop'),
		elements = {
			{label = _U('shop_1'), value = 'protein_shake'},
			{label = _U('shop_2'), value = 'water'},
			{label = _U('shop_3'), value = 'sportlunch'},
			{label = _U('shop_4'), value = 'powerade'},
			{label = _U('shop_5'), value = 'bandage'},
		}
	},function(data, menu)
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open('default',GetCurrentResourceName(),'gym_ship_menu',{
		title = _U('title') .. ' - ' .. _U('business_hours'),
		elements = {{label = _U('shop_6'), value = 'membership'},}
	},function(data, menu)
...
function OpenBikeMenu()
    ...
    ESX.UI.Menu.Open('default',GetCurrentResourceName(),'bike_menu',{
		title = _U('hire_a_bike'),
		elements = {
			{label = _U('shop_7'), value = 'bmx'},
			{label = _U('shop_8'), value = 'cruiser'},
			{label = _U('shop_9'), value = 'fixter'},
			{label = _U('shop_10'), value = 'scorcher'},
		}
	},function(data, menu)
...
RegisterNetEvent('esx:showGymNotification')
AddEventHandler('esx:showGymNotification',function(msg, ...)
